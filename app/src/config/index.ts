...
export function getModelCatalog(): ModelMeta[] {
  return [
    ...
    { id: 'claude-sonnet-5',   label: 'Sonnet 5',          note: '1M · Balanced', provider: 'anthropic' },
    ...
    { id: 'claude-opus-4-5',   label: 'Opus 4.5',         note: '1M · Max',      provider: 'anthropic' },
    { id: 'claude-sonnet-4-6', label: 'Sonnet 4.6',       note: '1M · High',     provider: 'anthropic' },
    { id: 'claude-sonnet-4-5', label: 'Sonnet 4.5',       note: '200K · High',   provider: 'anthropic' },
    ...
    { id: 'gpt-4.1-mini',      label: 'GPT-4.1 Mini',      note: 'OpenAI · Fast', provider: 'openai'    },
    { id: 'gpt-4.1-nano',      label: 'GPT-4.1 Nano',      note: 'OpenAI · Cheapest', provider: 'openai' },
 ...
 * Confirmed members (2026-08): fable-5, mythos-5, opus-5, sonnet-5, opus-4-8, opus-4-7, opus-4-6, sonnet-4-6.
...
export function supportsLongContext(model?: string): boolean {
  ...
  return /(?:^|-)(fable-5|mythos-5|opus-5|sonnet-5|opus-4-8|opus-4-7|opus-4-6|sonnet-4-6)(?:-|$)/.test(m);
}
...
export function getMaxOutput(model?: string): number {
  ...
  // Sonnet 5 lifts output to 128k; Sonnet 4.x and earlier stay at 64k.
  if (/(?:^|-)sonnet-5(?:-|$)/.test(m)) return 128_000;
