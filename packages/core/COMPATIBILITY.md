...
| **0.11.0** | + `AISession.setStickyParams?` + `AISession.peekParams?` | ✅ Unchanged | Additive. Both optional — providers/plugins that don't implement them keep working (callers use `?.`). Sticky per-session provider-interpreted params map (Anthropic `effort`, OpenAI `reasoning_effort`); router/UI transport it blind. Effort selector (mission Gearbox) uses it to apply per-session reasoning effort on the live session. Mirrors 0.5.0 `setStickyModelOverride?`. |
